<div align="center">
  ...
  [![Live Demo](https://img.shields.io/badge/🚀_Live_Demo-Visit_Site-success?style=for-the-badge)](https://videotourl.com/videos/1788624760922-0b0c15ae-2000-4e4b-a202-265c4f842baf.mp4)
  [![GitHub](https://img.shields.io/badge/GitHub-Repository-181717?style=for-the-badge&logo=github)](https://github.com/Mie-Yamin/Zura-UMKM.git)
...
<div align="center">
  <img src="https://img.sanishtech.com/u/d8c1a46d5436352544cd324228d744f0.png" alt="Homepage" width="800"/>
  <p><em>Homepage - Tampilan utama aplikasi</em></p>
  
  <img src="https://img.sanishtech.com/u/6d2a274bcccc7a76d22bad13d0b8fbd5.png" alt="Dashboard" width="800"/>
  <p><em>Dashboard - Panel kontrol pengguna</em></p>
  
  <img src="https://img.sanishtech.com/u/ecb4776cb0633e4c03995429efa59a48.png" alt="Feature" width="800"/>
...
graph TD
    %% User Layer
    User[👤 Pengguna / Pelaku UMKM] -->|Akses Dashboard| WebApp[🌐 React Single Page App]

    %% Frontend App (Client Side)
    subgraph Frontend [Frontend App - Vite + React]
        WebApp --> Router[React Router DOM]
        WebApp --> UI[Tailwind CSS & Lucide Icons]
        WebApp --> DataVis[Recharts & Leaflet Maps]
        WebApp --> Export[ExcelJS & html2pdf.js]
        WebApp --> Query[TanStack React Query]
    end

    %% Firebase Services (Backend / BaaS)
    Query -->|Data Fetching / Mutations| Firestore[(🔥 Firebase Firestore DB)]
    WebApp -->|Authentication| Auth[🔐 Firebase Auth]

    %% Build & Hosting
    Developer[👨‍💻 Developer] -->|Build / Test| Vitest[🧪 Vitest & Testing Library]
    Developer -->|Deploy| Vercel[🚀 Vercel / Netlify / Firebase Hosting]
...
erDiagram
    ...
    USERS ||--o{ RECAPS : "records"
    USERS ||--o{ TRANSACTIONS : "performs"
    RECAPS ||--|{ RECAP_ITEMS : "contains"

    USERS {
        string uid PK "Firebase Auth UID"
        string name "Nama Pemilik Toko"
        string email "Email Akun Google"
        string phone "Nomor Kontak / WhatsApp"
        string storeName "Nama Usaha UMKM"
        string address "Alamat Operasional Toko"
        string category "Kategori Industri UMKM"
        string updatedAt "ISO8601 Timestamp"
    }

    SETTINGS {
        string uid PK "Firebase Auth UID (1:1)"
        string userId FK "Foreign Key -> USERS.uid"
        int lowStockThreshold "Ambang Batas Minimum Peringatan Stok"
    }
    ...
    PRODUCTS {
        ...
        string sku "Kode Unik Produk (ZR-XX-XXXX)"
        string category "Kategori Barang"
        number purchasePrice "Harga Beli Pokok (HPP)"
        number sellingPrice "Harga Jual Satuan"
        int stockCount "Jumlah Stok Fisik Aktif"
        int minStock "Ambang Batas Stok Kritis"
        string status "Status Ketersediaan (AMAN / HABIS)"
    ...
    RECAPS {
        string id PK "Firestore Auto-generated ID"
        string userId FK "Foreign Key -> USERS.uid"
        string recapDate "Format YYYY-MM-DD"
        number totalAmount "Akumulasi Omzet Penjualan Harian"
        int unitsSold "Total Fisik Barang Terjual"
        string createdAt "ISO8601 Timestamp"
    }

    RECAP_ITEMS {
        string name "Nama Barang Terjual"
        int qty "Kuantitas Barang Terjual"
        number price "Harga Jual saat Transaksi"
    }

    TRANSACTIONS {
        string id PK "Firestore Auto-generated ID"
        string userId FK "Foreign Key -> USERS.uid"
        number amount "Nominal Arus Kas"
        string type "IN (Pemasukan) / OUT (Pengeluaran)"
        timestamp transactionDate "Waktu Transaksi Kasir"
        timestamp createdAt "Waktu Dokumen Dibuat"
    }
```
